(function () {
  ...
  function setLoading(loading) {
    if (loading) {
      ...
      generateBtn.textContent = "Generate Word Search PDF";
    }
  }
  ...
  form.addEventListener("submit", async function (event) {
    ...
    try {
      const response = await fetch("/word-search-builder/generate", {
        method: "POST",
      ...
      showStatus("success", [payload.message || "Word Search PDF created successfully."]);
      if (payload.warnings && payload.warnings.length) {
        showStatus("warning", payload.warnings);
      }
